...
abstract class DataObject {
    ...
    /**
     * @throws Exception default implementation
     * @returns string
     */
    public static function getTableName() {
        return 'Concrete DataObjects must override the static method getTableName';
    }
     ...
     * @param string $col
     * @param int $type
     * @param string $dbDefault IGNORED allowing DB to set default value upon commit
     */
    ...
    function getId() {
        return (int)$this->id;
        //return (int)($this->_data['id']->value);
    }
	...
	function checkSave(){
		if(!$this->save()) { 
			 throw new ApiException('Server Side Error');
		}
	}
    function copy() {
        $copy = clone $this;
        $copy->id = 0;
        foreach ($copy->_data as $col => &$value) {
            $value->flags |= DATAFLAG_CHANGED;
        } // set changed flags
    ...
    function load($cols = array()) {
        ...
        if (Config::$debug) {
            ...
            if ($id == 0) {
                throw new Exception("Assertion Error: Cannot load a data object (" . get_class($this) . ") with an id of 0");
            }

            // verify we asked only for existing columns
            foreach ($cols as $col) {
                if (!isset($this->_data[$col])) {
                    throw new Exception("Column '$col' of DataObject '" . $this->getObjectName() . "' not defined in load().");
                }
            }
        }
    ...
    function loadFromRow($row, $setChanged = false) {
        if (!$row) {
            return;
        }
            ...
            if (isset($this->_data[$col])) {
                ...
                if (is_null($value)) {
                    ...
                    if (($data->dataType == DATATYPE_DATETIME || $data->dataType == DATATYPE_DATE) && is_string($value)) {
                        $data->value = Database::readDate($value);
//                        if (strncmp($value, '0000-00-00', 10) == 0) {
//                            $data->value = 0;
//                        } else {
//                            $data->value = strtotime($value);
//                        }
                    } else {
                        if ($data->dataType == DATATYPE_INT && $value === (string)(int)$value) {
                            $data->value = (int)$value;
                        } else {
                            if ($data->dataType == DATATYPE_JSON) {
     ...
     * @return DataObject[]
     */
    public static function getByFields($fieldSet, $condition = '') {
